...
class DataItem {
  ...
  dbBrokenReferenceError() {
    return {
      statusCode: 409,
      body: JSON.stringify({
        message: `Could not find associated reference`
      })
    }
  }
  ...
  handleSqlError(error) {
    ...
    switch(errorCode) {
      ...
      case 1062:
        ...
        break;
      case 1452:
        _error = this.dbBrokenReferenceError();
        break;
      default:
  ...
  deleteById(id) {
    ...
    return new Promise((resolve, reject) => {
      this.getConnection().then((connection) => {
        ...
        let escapedId = connection.escape(id);

        let query = `DELETE FROM ${this.tableName} WHERE id=${escapedId}`;
